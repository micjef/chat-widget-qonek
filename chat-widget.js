...
(function(window) {
  function initChatWidget(config) {
    ...
    if (!firebaseConfig || !groupId || !username) {
      console.error("Firebase configuration, groupId, and username are required.");
      return;
    }
    ...
    db.collection("messages").where("groupId", "==", groupId).orderBy("timestamp").onSnapshot((snapshot) => {
      chatMessages.innerHTML = '';
      snapshot.forEach((doc) => {
        const data = doc.data();
        const messageElement = document.createElement('div');
        messageElement.className = data.user === username ? 'flex justify-end mb-3' : 'flex mb-3';
        messageElement.innerHTML = `<div class="${data.user === username ? 'bg-gray-800 text-white' : 'bg-gray-200 text-black'} rounded-lg py-2 px-4 max-w-[70%]">${data.message}</div>`;
        chatMessages.appendChild(messageElement);
      });
      chatMessages.scrollTop = chatMessages.scrollHeight;
    });
